...
float GetWaves(vec3 position) {
    ...
    mat2 rot1 = mat2( cos(0.1), sin(0.1), -sin(0.1), cos(0.1) );
    mat2 rot2 = mat2( cos(-0.3), sin(-0.3), -sin(-0.3), cos(-0.3) );
    mat2 rot3 = mat2( cos(0.5), sin(0.5), -sin(0.5), cos(0.5) );

    // 1. Welle
    vec2 q = rot1 * p;
    float wave = textureSmooth(noisetex, (q * vec2(2.0f, 1.2f)) + vec2(0.0f, q.x * 2.1f)).z;
    p /= 2.1f;
    p.y -= wavesTime / 20.0f;
    p.x -= wavesTime / 30.0f;
    allwaves += wave;
    ...
    weights += weight;
    q = rot2 * p;
    wave = textureSmooth(noisetex, (q * vec2(2.0f, 1.4f)) + vec2(0.0f, -q.x * 2.1f)).z;
    p /= 1.5f;
    p.x += wavesTime / 20.0f;
    wave *= weight;
    allwaves += wave;
    ...
    weights += weight;
    q = rot3 * p;
    wave = textureSmooth(noisetex, (q * vec2(1.0f, 0.75f)) + vec2(0.0f, q.x * 1.1f)).x;
    p /= 1.5f;
    p.x -= wavesTime / 55.0f;
    wave *= weight;
    allwaves += wave;
    ...
    weights += weight;
    q = rot1 * p;
    wave = textureSmooth(noisetex, (q * vec2(1.0f, 0.75f)) + vec2(0.0f, -q.x * 1.7f)).x;
    p /= 1.9f;
    p.x += wavesTime / 155.0f;
    wave *= weight;
    allwaves += wave;

    // 5. Welle (mit Verzerrung)
    weight = 29.25f;
    weights += weight;
    q = rot2 * p;
    wave = abs(textureSmooth(noisetex, (q * vec2(1.0f, 0.8f)) + vec2(0.0f, -q.x * 1.7f)).y * 2.0f - 1.0f);
    p /= 2.0f;
    p.x += wavesTime / 155.0f;
