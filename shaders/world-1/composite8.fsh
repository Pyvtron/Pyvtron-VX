...
vec3 SampleColor(vec2 coord) {
    return GammaToLinear(texture2D(colortex1, coord).rgb);
}

vec3 SamplePreviousColor(vec2 coord) {
    return GammaToLinear(texture2D(colortex7, coord).rgb);
}
...
float GetExposureTiles()
{
	...
	for(int y = 0; y < tileCount.y; y++)
	{
        ...
        float lumaWeight = pow(avg / tileExposure, 0.7);
...
void main() {
    vec4 taa;
    MaterialMask materialMask	= CalculateMasks(texture2D(colortex5, texcoord.st).b);
    ...
    float alphaPassthrough = texture2D(colortex7, texcoord).a;
